...
internal class TVAccountStateConverter : JsonConverter<TvAccountState>
{
    ...
    public override void Write(Utf8JsonWriter writer, TvAccountState value, JsonSerializerOptions options)
    {
        ...
        var jsonObject = JsonSerializer.SerializeToNode(value, SourceGenerationContext.Default.TvAccountState)?.AsObject();
        ...
        if (jsonObject != null)
        {
            ...
            {
                jsonObject["rated"] = JsonSerializer.SerializeToNode(new { value = contentRating }, SourceGenerationContext.Default.TvAccountState);
            }
            else
            {
